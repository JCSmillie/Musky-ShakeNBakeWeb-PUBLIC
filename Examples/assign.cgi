...
log_line() {
    ...
    #echo "$TODAY =====>$LINE"
    #Log to file
    echo "iPadAssign.sh ++> $TODAY =====> $LINE" >> ../../../ConfigFiles/ipadactions.txt
}
...
echo ""
echo "<html><head><title>iPad Sign In Check"
...
#	HayLookAtMe "ACTION variable set to NOASSIGN.  Check logs...  FAIL"
	...
	echo '<body style="background-color:red;">'
	echo ' <font size="+4">'
	echo "<strong>IIQ says $ASSTAG is stolen..  Not Assigning... and forgetting I know you...</strong>"
	echo '</font>'

	exit 1

elif [ "$DeviceAssignd" ]; then

	USERIDperIIQ=$(echo "$DeviceAssignd" | cut -d '@' -f1 )
	log_line "Per IIQ $DeviceSerialNumber is assigned to  $USERIDperIIQ and listed as DEPLOYABLE."
	GetCurrentInfo-ios

	log_line "Mosyle says this device ($DeviceSerialNumber) is currently assigned to $USERID"

	if [ -z "$USERID" ] || [ "$USERID" = "UNASSIGNED" ]; then
		echo "Device CAN be assigned."
		AssigniPad

	else
		log_line "Device ($DeviceSerialNumber) cannot be assigned per Mosyle."
		exit 1
	fi

else
	log_line "IIQ lookup for $ASSTAG ($DeviceSerialNumber) gave no reason to assign by lookup.  IsStolen-> $IsStolen / DeviceAssignd-> $DeviceAssignd"
